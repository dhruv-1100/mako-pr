...
void TpccProcedure::Init(TxRequest &req) {
  Log_info("TpccProcedure::Init start");
  ws_init_ = req.input_;
  ws_ = ws_init_;
  type_ = req.tx_type_;
  ...
  if (txn_reg_.expired()) {
      Log_fatal("txn_reg_ expired");
  }
  auto reg = txn_reg_.lock();
  if (!reg) {
      Log_fatal("txn_reg_ lock failed");
  }
  leaf_procs_ = reg->regs_[type_];
  Log_info("TpccProcedure::Init leaf_procs_ retrieved");
  ...
  Log_info("TpccProcedure::Init end");
}
...
bool TpccProcedure::CheckReady() {
  ...
  for (auto &pair : status_) {
    ...
    // Log_info("CheckReady checking piece %d", pi);
